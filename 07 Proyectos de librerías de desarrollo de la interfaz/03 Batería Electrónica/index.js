...
const DumControle = ({ stop, name, power, volume, handleVolumeChange, changeSoundGroup }) => (
    <div className="controle">
        ...
        <button onClick={changeSoundGroup}>Cambio Grupo de Sonidos</button>
    </div>
);
...
const App = () => {
    ...
    const styleActiveKey = (key) => {
        ...
        key.parentElement.style.color = "#e3ff66"
    }

    const deActivatedKey = (audio) => {
        audio.parentElement.style.backgroundColor = "#ffffff"
        audio.parentElement.style.color = "#274018"
    }

    const deactivateAudio = (audio) => {
        setTimeout(() => {
            audio.parentElement.style.backgroundColor = "#e3ff66"
            audio.parentElement.style.color = "#274018"
        }, 300)
    }
